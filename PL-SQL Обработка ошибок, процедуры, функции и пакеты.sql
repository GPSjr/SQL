...
Входящий параметр: P_JOB_ID Общая логика процедуры: Удаление существующей должности в справочнике HR.JOBS_INICIALY (создать свою копию таблицы).
Исключение: Процедура должна проверить существование удаляемой должности, если должности нет,
выдаем сообщение “Данной должности не существует” и останавливаем дальнейшие действия.
Действие: Удаление должности по P_JOB_ID из  HR.JOBS_INICIALY. В конце удаление нужно поставить commit;
...
Входящий параметр: P_JOB_ID
Общая логика функции: Функция должна проверить по коду должности, есть ли такая должность в таблице сотрудников или в таблице должностей. 
Действие/проверка: Если должность, есть на сотруднике (таблица employees), вернуть (через return) “Данная должность есть на сотруднике”.
...
BEGIN
     ...
     BEGIN
        ...
        WHERE EM.EMPLOYEE_ID = P_EMP_ID;
     EXCEPTION WHEN OTHERS THEN V_REGION_NAME:='ÍÅÒ ÒÀÊÎÃÎ ÑÎÒÐÓÄÍÈÊÀ';
     END;
